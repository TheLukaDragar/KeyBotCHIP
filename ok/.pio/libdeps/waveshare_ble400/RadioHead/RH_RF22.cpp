...
bool RH_RF22::init(void (*callbackPtr)(String ) )
{
#if RH_PLATFORM == RH_PLATFORM_ESP8266
    flagIsr[0] = false;
    flagIsr[1] = false;
    ...
    (*callbackPtr) ("unknown device type: ");
...
void RH_RF22::handleInterrupt()
{
    ...
    Serial.println("PKVALID");
    if (_lastInterruptFlags[0] & RH_RF22_ICRCERROR)
    Serial.println("CRCERROR");
    if (_lastInterruptFlags[0] & RH_RF22_IFFERROR)
    Serial.println("FFERROR");
    if (_lastInterruptFlags[0] & RH_RF22_IPREAINVAL)
    Serial.println("PREAINVAL");
    if (_lastInterruptFlags[0] & RH_RF22_IPREAVAL)
    Serial.println("PREAVAL");
    if (_lastInterruptFlags[0] & RH_RF22_IRSSI)
    Serial.println("RSSI");
    if (_lastInterruptFlags[0] & RH_RF22_IWUT)
    Serial.println("WUT");
    
    if (_lastInterruptFlags[1] & RH_RF22_ILBD)
    Serial.println("LBD");
    if (_lastInterruptFlags[1] & RH_RF22_ICHIPRDY)
    Serial.println("CHIPRDY");
    if (_lastInterruptFlags[1] & RH_RF22_IPOR)
    Serial.println("POR");
    if (_lastInterruptFlags[1] & RH_RF22_IEXT)
    Serial.println("EXT");

    //read status with statusRead();
    int status = statusRead();
